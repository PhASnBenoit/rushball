...
int CI2c::lire(unsigned char addr,  unsigned char *buffer, size_t lg)
{
    ...
    ssize_t nb=read(m_fileI2c, buffer, lg);
    return  nb;
} // lire

int CI2c::ecrire(unsigned char addr, unsigned char *buffer, size_t lg)
{
    if(ioctl(m_fileI2c, I2C_SLAVE, addr)!=0) {  // Règle le driver I2C sur l'adresse.
    ...
    ssize_t nb=write(m_fileI2c, buffer, lg);
    return nb;
} // ecrire
...
{
    char filename[40];
    sprintf(filename,"/Applications/appRushball-I2C/i2c-%c",m_noBus);
